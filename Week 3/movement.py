...
LOWER_RED1 = np.array([0, 150, 70])  # Updated for red values (R:180-200, G:10-20, B:35-50)
UPPER_RED1 = np.array([10, 255, 200])
LOWER_RED2 = np.array([170, 150, 70])  # Red wraps around HSV space
UPPER_RED2 = np.array([180, 255, 200])
LOWER_BLUE = np.array([100, 100, 70])  # Updated for blue values (R:5-20, G:30-40, B:80-90)
UPPER_BLUE = np.array([120, 255, 150])
#LOWER_GREEN = np.array([60, 100, 100])  # Updated for green values (R:20, G:140-150, B:100-110)
#UPPER_GREEN = np.array([90, 255, 200])
LOWER_YELLOW = np.array([20, 150, 150])  # Updated for yellow values (R:195-225, G:150-190, B:30-40)
UPPER_YELLOW = np.array([35, 255, 255])

#LOWER_BLUE = np.array([100, 150, 60])
...
def orb_detect_symbols(frame_bgr, orb, bf, symbol_features, prev_detected_symbol=None):
    ...
    if max_matches > detection_threshold:
        ...
        if candidate_symbol != prev_detected_symbol:
            ...
            #if candidate_symbol == 'stop' or candidate_symbol == 'hand stop':
             #   stop_motors(right_pwm, left_pwm)
              #  time.sleep(3)
...
def main():
    global right_pwm, left_pwm, servo_pwm
    ...
    try:
        while True:
            ...
            # Always perform line detection
            line_detections = detect_lines(frame_bgr)
            output = preprocess(frame_bgr)
            if output:
                if output == 'handSign' or output == 'stop':
                    stop_motors(right_pwm, left_pwm)
                    time.sleep(3)
            ...
            if best_symbol:
                ...
                # Removed face recognition logic

            # Display status and detected lines for better debugging
            ...
            # Draw contours for the target color if found
            if target_found and target_color:
                center_x = line_detections[target_color]['center_x']
                if center_x is not None:
                    cv2.circle(frame_bgr, (center_x, FRAME_HEIGHT // 2), 5, (0, 255, 0), -1)
                    cv2.line(frame_bgr, (FRAME_WIDTH // 2, FRAME_HEIGHT // 2), 
                             (center_x, FRAME_HEIGHT // 2), (0, 255, 0), 2)
                    
                    error_text = f"Error: {target_error}"
                    cv2.putText(frame_bgr, error_text, (10, FRAME_HEIGHT - 30),
